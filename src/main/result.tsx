...
import { getWallet, getNetworkType } from '@disperse/store';
import { viewOnExplorerByTxHash, parseContractError } from '@disperse/global';
...
export class Result extends Module {
  ...
  async renderUI() {
    ...
    if (this.message.status === 'warning') {
        ...
        <i-panel height={100}>
          <i-vstack id="loadingElm" class="i-loading-overlay" height="100%" background={{color: "transparent"}}>
            <i-vstack class="i-loading-spinner" horizontalAlignment="center" verticalAlignment="center">
              ...
              ></i-icon>
              <i-label caption="Loading..." font={{ color: Theme.colors.primary.main }} class="i-loading-spinner_text"></i-label>
            </i-vstack>
          </i-vstack>
        </i-panel>
      )
      ...
      const options = [ {font: { color: Theme.colors.warning.light, size: '20px'}, marginBottom: 15}, { marginBottom: 15 }, { font: { color: '#C2C3CB' }}];
      ...
      const label = await Label.create({
        ...
        margin: {bottom: 10},
        font: { size: '20px', color: Theme.colors.warning.light }
      });
      ...
      const label = await Label.create({
        ...
        font: { size: '18px', color: Theme.colors.warning.light },
        margin: {bottom: 10}
      });
      mainSection.appendChild(label);
