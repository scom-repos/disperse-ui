import { application, Button, Container, customModule, EventBus, HStack, Image, Input, Label, Module, Panel, Styles, VStack, Modal } from '@ijstech/components'
import { TokenSelection } from '@disperse/common';
import { EventId, ITokenObject, toDisperseData, downloadCSVFile, formatNumber, viewOnExplorerByTxHash, isAddressValid, DisperseData, disperseDataToString, registerSendTxEvents, formatUTCDate } from '@disperse/global';
import { getTokenBalance, getTokenIconPath, Networks, setTokenBalances, dummyAddressList, ImportFileWarning } from '@disperse/store';
import { BigNumber, Wallet } from '@ijstech/eth-wallet';
import { onApproveToken, onCheckAllowance, onDisperse, getDisperseAddress } from '@disperse/disperse-utils';
import { Result } from '@disperse/main/result';
...
export class Disperse extends Module {
  ...
  private onChainChanged = () => {
    this.resetData();
    this.onWalletConnect(Wallet.getInstance().isConnected);
  }
  ...
  get balance() {
    if (this.token) {
      return getTokenBalance(this.token);
    }
    return '0';
  }
  ...
  private setFourthStatus = async () => {
    if (Wallet.getInstance().isConnected && this.hasAddress) {
      this.btnDownload.enabled = true;
      this.containerElm.minHeight = '1050px';
      ...
      for (let i = 0; i < this.listAddresses.length; i++) {
        const item = this.listAddresses[i];
        ...
        this.addressesElm.appendChild(
          <i-hstack width="100%" verticalAlignment="center" gap={15}>
            <i-label width={30} caption={`${i + 1}`} font={{ size: '14px', color: '#A8A8A8', name: 'Montserrat' }} />
            <i-vstack width={370}>
              <i-label caption={item.address} font={{ size: '14px', color: valid ? '#A8A8A8' : '#F05E61', name: 'monospace' }} />
            </i-vstack>
            <i-label width={160} caption={item.amount.toFixed()} font={{ size: '14px', color: '#A8A8A8', name: 'Montserrat' }} />
            <i-label width={80} caption={symbol} font={{ size: '14px', color: '#A8A8A8', name: 'Montserrat' }} />
          </i-hstack>
        );
      };
  ...
  private updateNetworkBtn = async (connected: boolean) => {
    ...
    const network = Networks[Wallet.getInstance().chainId];
  ...
  private handleDisperse = async () => {
    ...
    let receipt = '';
    let timestamp = '';
    const address = Wallet.getInstance().address;
    ...
    this.resultElm.appendChild(
      <i-vstack gap={50} horizontalAlignment="center">
        ...
        <i-hstack gap={30} maxWidth="100%" horizontalAlignment="center" verticalAlignment="center">
          ...
          <i-button caption="DOWNLOAD REPORT" background={{color: Theme.background.gradient}} width={270} border={{ radius: 12 }} padding={{top: 12, bottom: 12}} onClick={() => this.onDownloadReport({ symbol: token.symbol, ...data })} />
  ...
  init() {
    super.init();
    const connected = Wallet.getInstance().isConnected;
    this.checkStepStatus(connected);
    this.updateNetworkBtn(connected);
    this.updateUserInfo(connected);
  ...
  render() {
    return (
      <i-panel class="template-layout disperse-component">
        ...
        <i-panel class="container-layout">
          ...
          <i-vstack id="containerElm" position="relative" margin={{top: 35}} minHeight={800}>
            ...
            <i-hstack id="secondStepElm" class="step-elm" minHeight={160} margin={{top: 35}} padding={{left: 50, right: 50}} border={{ radius: 30 }} verticalAlignment="center" horizontalAlignment="space-between" background={{color: Theme.background.modal}}>
              ...
              <i-hstack class="step-2" width="calc(100% - 470px)">
                <i-hstack id="tokenElm" width="100%" wrap="nowrap" verticalAlignment="center" horizontalAlignment="space-between" padding={{top: 20, bottom: 20, left: 60, right: 60 }} border={{ radius: 15, style: 'solid', width: 4 }}>
                  ...
                  <i-icon name="caret-down" fill={Theme.colors.primary.main} width={24} height={24} />
                </i-hstack>
            ...
            <i-vstack id="fourthStepElm" class="step-elm" minHeight={240} margin={{top: 35}}>
              <i-vstack width="100%" background={{color: "#34343A"}} border={{ radius: 30 }}>
                <i-hstack verticalAlignment="center" horizontalAlignment="space-between">
                  <i-vstack class="step-4" background={{color: Theme.background.modal}} width={800} height="100%" padding={{ top: 20, bottom: 20, left: 50, right: 50 }} gap={15} border={{ radius: 30 }}>
                    <i-hstack width="100%" verticalAlignment="center" gap={15}>
                      <i-label caption="STEP 4" font={{ size: '20px', name: 'Montserrat SemiBold', color: Theme.colors.primary.main }} />
                    ...
                    <i-vstack width="100%" verticalAlignment="center" gap={10} class="custom-scroll">
                      ...
                      <i-vstack gap={10} class="address-elm">
                        <i-hstack width="100%" verticalAlignment="center" gap={15}>
                          <i-label width={30} caption="#" font={{ size: '16px', name: 'Montserrat Medium' }} />
                          <i-vstack width={370}>
                            <i-label caption="Addresses" font={{ size: '16px', name: 'Montserrat Medium' }} />
                          </i-vstack>
                          <i-label width={160} caption="Amount" font={{ size: '16px', name: 'Montserrat Medium' }} />
                          <i-label width={80} caption="Token" font={{ size: '16px', name: 'Montserrat Medium' }} />
                        </i-hstack>
                        <i-vstack width="100%" maxHeight={210} class="overflow-auto" padding={{right: 5}}>
              ...
              <i-hstack verticalAlignment="center" horizontalAlignment="center" margin={{top: 35}} gap={30}>
                <i-button
                  id="btnApprove"
                  caption="Approve"
                  class="btn-os"
