...
import { getSlippageTolerance, setSlippageTolerance, setTransactionDeadline, getTransactionDeadline, isExpertMode, toggleExpertMode } from '@disperse/store';
import styleClass from './transactionSettingsLayout.css';

declare global {
	namespace JSX {
		interface IntrinsicElements {
			['transaction-settings-layout']: ControlElement;
		}
	}
};
...
@customElements('transaction-settings-layout')
export class TransactionSettingsLayout extends Module {
  ...
  blurSlippageTolerance = (source: Control, val: any) => {
    if (val && val >= 50) {
  ...
  inputDeadline = (source: Control, val: any) => {
  ...
  blurTransactionDeadline = (source: Input, val: any) => {
      ...
      <i-panel class="settings-content">
        ...
        <i-hstack id="slippageGroup" gap=".5rem">
          <i-input
            ...
            onChange={this.inputSlippageTolerance}
