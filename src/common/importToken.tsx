...
import { EventId, ITokenObject, viewOnExplorerByAddress } from '@disperse/global';
import { addUserTokens, setTokenBalances, setTokenMap } from '@disperse/store';
...
export class ImportToken extends Module {
  ...
  render() {
      ...
      <i-modal id="importModal" class="bg-modal" title="Select Token" closeIcon={{ name: 'times' }}>
        <i-panel margin={{top: 12, bottom: 12}} padding={{top: 20, bottom: 16, left: 16, right: 16}} border={{radius: 12, width: 2, style: 'solid', color: Theme.colors.secondary.main}}>
          <i-panel>
            <i-icon name="question" class="cicrle" fill={Theme.colors.primary.main} width={15} height={15} margin={{right: 4}} />
          ...
          <i-panel padding={{ top: 5, bottom: 5, left: 5, right: 5}} display="inline-block" background={{color: Theme.background.gradient}} border={{ radius: 5 }}>
        ...
        <i-panel margin={{top: 12, bottom: 12}} padding={{top: 20, bottom: 16, left: 16, right: 16}} border={{radius: 12, width: 2, style: 'solid', color: Theme.colors.secondary.main}}>
          <i-hstack horizontalAlignment="center" margin={{bottom: 5}}> 
            <i-icon name="exclamation-triangle" margin={{right: 4}} fill={Theme.colors.primary.main} width={30} height={30} />
          </i-hstack>
