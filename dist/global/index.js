define("@disperse/global",(require, exports)=>{
...
__export(exports, {
  EventId: () => EventId,
  SITE_ENV: () => SITE_ENV,
  disperseDataToString: () => disperseDataToString,
  downloadCSVFile: () => downloadCSVFile,
  explorerAddressUrlsByChainId: () => explorerAddressUrlsByChainId2,
  explorerTxUrlsByChainId: () => explorerTxUrlsByChainId2,
  ...
  renderBalanceTooltip: () => renderBalanceTooltip,
  toDisperseData: () => toDisperseData,
  toWeiInv: () => toWeiInv,
  uniqWith: () => uniqWith,
...
var formatUTCDate = (date, formatType = DefaultDateFormat) => {
  const formatted = (0, import_moment.default)(date).format(formatType);
  return `${formatted} (UTC+${(0, import_moment.default)().utcOffset() / 60})`;
};
...
var formatPercentNumber = (value, decimals) => {
  let val = value;
  if (typeof value === "string") {
    val = new import_eth_wallet.BigNumber(value).toNumber();
  } else if (typeof value === "object") {
...
var isInvalidInput = (val) => {
  const value = new import_eth_wallet.BigNumber(val);
  if (value.lt(0))
    return true;
  return (val || "").toString().substring(0, 2) === "00" || val === "-";
};
var limitInputNumber = (input, decimals) => {
  const amount = input.value;
  if (isInvalidInput(amount)) {
    input.value = "0";
    return;
  }
  if (!new import_eth_wallet.BigNumber(amount).isNaN()) {
    input.value = limitDecimals(amount, decimals || 18);
  }
};
var limitDecimals = (value, decimals) => {
  let val = value;
  if (typeof value !== "string") {
    val = val.toString();
  }
...
var viewOnExplorerByTxHash = (chainId, txHash) => {
  if (import_global.explorerTxUrlsByChainId[chainId]) {
    let url = `${import_global.explorerTxUrlsByChainId[chainId]}${txHash}`;
    window.open(url);
  }
};
var viewOnExplorerByAddress = (chainId, address) => {
  if (import_global.explorerAddressUrlsByChainId[chainId]) {
    let url = `${import_global.explorerAddressUrlsByChainId[chainId]}${address}`;
    window.open(url);
  }
};
var toWeiInv = (n, unit) => {
  if (new import_eth_wallet.BigNumber(n).eq(0))
    return new import_eth_wallet.BigNumber("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
  return new import_eth_wallet.BigNumber("1").shiftedBy((unit || 18) * 2).idiv(new import_eth_wallet.BigNumber(n).shiftedBy(unit || 18));
};
var padLeft = function(string, chars, sign) {
  return new Array(chars - string.length + 1).join(sign ? sign : "0") + string;
};
var numberToBytes32 = (value, prefix) => {
  if (!value)
    return;
  let v = value;
  if (typeof value == "number") {
    ...
    v = new import_eth_wallet.BigNumber(value).toString(16);
  } else if (/^(0x)?[0-9A-Fa-f]*$/.test(value)) {
    v = value;
  } else if (import_eth_wallet.BigNumber.isBigNumber(value)) {
    v = value.toString(16);
  }
  v = v.replace("0x", "");
  v = padLeft(v, 64);
...
var formatNumberValue = (data, tokenMap) => {
  const { title, value, symbol, icon, prefix, isWrapped } = data;
  try {
    let limitDecimals2 = 18;
    if (symbol) {
      let symb = symbol;
      if (symb.includes("/")) {
        symb = symb.split("/")[0];
      ...
      if (symbol === "USD") {
        ...
        if (tokenObj) {
          limitDecimals2 = tokenObj.decimals || 18;
        }
      }
    }
    ...
    if (val === 0) {
      ...
      tooltip = val.toLocaleString("en-US", { maximumFractionDigits: limitDecimals2 });
    } else {
      const stringValue = value.toString();
      const decimalsIndex = stringValue.indexOf(".");
      const length = decimalsIndex < 0 ? stringValue.length : stringValue.length - 1;
      let valueFormatted = val.toLocaleString("en-US", { maximumFractionDigits: limitDecimals2 });
      ...
      if (parts[1]) {
        let decVal = parts[1];
        if (parts[1].length > limitDecimals2) {
          decVal = parseFloat(`0.${parts[1]}`).toLocaleString("en-US", { maximumFractionDigits: limitDecimals2 });
          if (decVal == 1) {
            decVal = parts[1].substr(0, limitDecimals2);
          } else {
...
var renderBalanceTooltip = (params, tokenMap, isBold) => {
...
function isAmount(text) {
  return !new import_eth_wallet.BigNumber(text).isNaN();
}
...
var toDisperseData = (inputText) => {
  ...
  var TextType;
  (function(TextType2) {
    TextType2[TextType2["Address"] = 0] = "Address";
    TextType2[TextType2["Amount"] = 1] = "Amount";
    TextType2[TextType2["Other"] = 2] = "Other";
    TextType2[TextType2["None"] = 3] = "None";
  })(TextType || (TextType = {}));
    ...
    switch (thisText) {
      case 0: {
        if (savedAddress)
          data.push({
            ...
            amount: new import_eth_wallet.BigNumber("0")
          });
      ...
      case 1: {
        if (savedAddress) {
          data.push({
            address: savedAddress,
            amount: new import_eth_wallet.BigNumber(textList[i])
          });
        } else {
          data.push({
            address: "0x",
...
var downloadCSVFile = (content, name) => {
...
var isTransactionConfirmed = async (txHash) => {
  const tx = await import_eth_wallet2.Wallet.getInstance().web3.eth.getTransaction(txHash);
...
async function getERC20Amount(wallet, token, decimals) {
  let erc20 = new import_eth_wallet2.Erc20(wallet, token, decimals);
  return await erc20.balance;
}
var isAddressValid = async (address) => {
...
var explorerAddressUrlsByChainId2 = {
...
(function(EventId2) {
  ...
  EventId2["ChangeNetwork"] = "changeNetwork";
